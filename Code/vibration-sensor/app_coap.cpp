...
coapSender::coapSender()
{
	...
	mResource_PERMISSIONS.mHandler = &coapSender::permissionsHandlerStaticWrapper;
...
void coapSender::permissionsHandler(void *aContext, otMessage *aMessage,
...
bool coapSender::checkBuffer(void)
{

}

bool coapSender::send(bool require_ack) {
	//appCoapCheckConnection();
	if (!connectionEstablished || !payload_buf_valid)
		return false;
	// GPIO_PinOutSet(IP_LED_PORT, IP_LED_PIN);
	otError error = OT_ERROR_NONE;
	...
	payloadLength = strlen(this->payload_buf);
	...
	if (payloadLength > 0) {
		error = otMessageAppend(message, this->payload_buf, payloadLength);
		otEXPECT(OT_ERROR_NONE == error);
	}
...
bool coapSender::parseIntoBuffer(app_data_t *data, app_msg_t type) {
	...
	case MSG_FULL_SPECTRUM: {
		 // TODO eval break msg into N/MAX_MSG_LENGTH sections for complete
		 // transmission of the freq spectrum.
	...
	payload_buf_valid = true;
	return true;
}

bool coapSender::checkConnectionValid(void) {
